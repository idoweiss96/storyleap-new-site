...
import { Sparkles, BookOpen, ArrowLeft, ArrowRight, X, PartyPopper } from 'lucide-react';
...
export default function OnboardingTour({ onComplete, onSkip }) {
  ...
  const tourSteps = [
    { id: 'welcome', title: t('onb_welcome_title'), description: t('onb_welcome_desc'), icon: PartyPopper },
    { id: 'create', title: t('onb_create_title'), description: t('onb_create_desc'), icon: Sparkles },
    { id: 'stories', title: t('onb_stories_title'), description: t('onb_stories_desc'), icon: BookOpen },
    { id: 'finish', title: t('onb_finish_title'), description: t('onb_finish_desc'), icon: Sparkles },
  ];
        ...
        >
          <Card className="w-full max-w-md bg-white border-0 shadow-2xl overflow-hidden">
            <div className="bg-gradient-to-r from-violet-500 to-violet-600 p-6 text-white relative">
              <button
                onClick={onSkip}
                className={`absolute top-4 ${isRTL ? 'right-4' : 'left-4'} w-8 h-8 rounded-full bg-white/20 flex items-center justify-center hover:bg-white/30 transition-colors`}
              >
                <X className="w-4 h-4" />
              </button>
            ...
            <div className="p-6">
              ...
              <div className="flex gap-3">
                ...
                >
                  {isLast ? t('onb_go') : t('onb_next')}
                  {!isLast && (isRTL ? <ArrowLeft className="w-4 h-4 mr-1" /> : <ArrowRight className="w-4 h-4 ml-1" />)}
                </Button>
              </div>
